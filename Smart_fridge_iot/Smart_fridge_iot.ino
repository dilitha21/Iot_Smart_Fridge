...
#define MQ135_PIN 35                    // ESP32 GPIO 35 (MQ-135 Ammonia Sensor Analog Input)

// --- Buzzer and Alarm Settings ---
#define BUZZER_PIN 27                   // ESP32 GPIO 27 (Digital Output for Buzzer)
// Note: Only MQ-135 triggers the buzzer now
#define AMMONIA_ALARM_THRESHOLD 580     //  Raw ADC threshold to trigger the buzzer
#define ALCOHOL_THRESHOLD 2200          // Raw ADC threshold for high alcohol (spoiled food) warning (for Firebase only)
...
#define CALIBRATION_FACTOR -1040.0      // Placeholder. Replace after calibration

// --- I2C LCD DEFINITIONS ---
#define LCD_COLUMNS 16                  // 16 characters wide
#define LCD_ROWS 2                      // 2 rows high
#define LCD_ADDRESS 0x27                // Common I2C address (check your module)
...
bool g_alcoholAlarm = false, g_ammoniaAlarm = false, g_spoiledByWeight = false; // g_ammoniaAlarm controls the buzzer now


// Function to read MQ-3 sensor (Alcohol)
float readMQ3() {
    return (float)analogRead(MQ3_PIN);
}

// Function to read MQ-135 sensor (Ammonia)
float readMQ135() {
...
void updateDisplay() {
    ...
    if (g_ammoniaAlarm) { // *** Buzzer triggers this high priority screen ***
        lcd.setCursor(0, 0);
        lcd.print("!! AMMONIA ALERT !!");
        lcd.setCursor(0, 1);
        lcd.printf("Amm: %d > %d", (int)g_mq135, AMMONIA_ALARM_THRESHOLD);
    } 
    else if (g_alcoholAlarm) {
        // High Priority Spoilage Alert (No buzzer, but high priority on screen)
        lcd.setCursor(0, 0);
...
void loop() {
    ...
    if (millis() - lastSend > 5000) {
        ...
        // --- Alcohol Alarm Status (for Firebase only) ---
        g_alcoholAlarm = false;
        if (g_mq3 >= ALCOHOL_THRESHOLD) {
            g_alcoholAlarm = true;
        }
        
        // --- AMMONIA ALARM Logic (BUZZER CONTROL) ---
        g_ammoniaAlarm = false;
        if (g_mq135 >= AMMONIA_ALARM_THRESHOLD) { // Check against the new threshold 580
            g_ammoniaAlarm = true;
            Serial.println("!!! HIGH AMMONIA WARNING - Buzzer ON !!!");
        }
        setAlarm(g_ammoniaAlarm); // Control the physical buzzer based on ammonia only
        ...
        Serial.printf("Temp: %.1f C | Hum: %.1f | Alc: %.0f | Amm: %.0f | Amm Alarm: %s\n", 
                        g_t, g_h, g_mq3, g_mq135, g_ammoniaAlarm ? "ON" : "OFF");
